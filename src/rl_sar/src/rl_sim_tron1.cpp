#include "rl_sim_tron1.hpp"

// #define PLOT
// #define CSV_LOGGER
// #define ROS_LOGGER // Publish joint target position data (generated by the policy) to ROS topics TODO: ROS_LOGGER for tron1 is not used and completed
...
RL_Sim::RL_Sim()
{
    ...
    this->mapped_joint_efforts = std::vector<double>(this->params.num_of_dofs, 1.0);
    ...
    // Initilaze Publisher:  ROS_LOGGER 
#ifdef ROS_LOGGER
    this->joint_target_pos_publisher = nh.advertise<robot_msgs::A1JointPos>("/joint_target_pos", 10);
    this->joint_target_pos.FL_hip_pos = 0.0;
    this->joint_target_pos.FL_hip_pos = 0.0;
    this->joint_target_pos.FL_hip_pos = 0.0;
    this->joint_target_pos.FR_hip_pos = 0.0;
    this->joint_target_pos.FR_hip_pos = 0.0;
    this->joint_target_pos.FR_hip_pos = 0.0;

    this->joint_target_pos.RL_hip_pos = 0.0;
    this->joint_target_pos.RL_hip_pos = 0.0;
    this->joint_target_pos.RL_hip_pos = 0.0;
    this->joint_target_pos.RR_hip_pos = 0.0;
    this->joint_target_pos.RR_hip_pos = 0.0;
    this->joint_target_pos.RR_hip_pos = 0.0;
#endif

    // Initilaze Subscribers:  Subscribe to gazebo model states, joint states, and cmd_vel
    ...
    // Initilaze Service Clients:  Gazebo service clients
    ...
    // Initilaze Loop Functions:  Loop functions for control, rl, and keyboard
    ...
    // Printing Initialization Done Message
    std::cout << LOGGER::INFO << "RL_Sim_TRON1 Started Successfully!!!" << std::endl;
}
...
#endif
#ifdef ROS_LOGGER
        this->joint_target_pos.FL_hip_pos = this->output_dof_pos[0][0].item<double>();
        this->joint_target_pos.FL_hip_pos = this->output_dof_pos[0][1].item<double>();
        this->joint_target_pos.FL_hip_pos = this->output_dof_pos[0][2].item<double>();
        this->joint_target_pos.FR_hip_pos = this->output_dof_pos[0][3].item<double>();
        this->joint_target_pos.FR_hip_pos = this->output_dof_pos[0][4].item<double>();
        this->joint_target_pos.FR_hip_pos = this->output_dof_pos[0][5].item<double>();

        this->joint_target_pos.RL_hip_pos = this->output_dof_pos[0][6].item<double>();
        this->joint_target_pos.RL_hip_pos = this->output_dof_pos[0][7].item<double>();
        this->joint_target_pos.RL_hip_pos = this->output_dof_pos[0][8].item<double>();
        this->joint_target_pos.RR_hip_pos = this->output_dof_pos[0][9].item<double>();
        this->joint_target_pos.RR_hip_pos = this->output_dof_pos[0][10].item<double>();
        this->joint_target_pos.RR_hip_pos = this->output_dof_pos[0][11].item<double>();

        // this->joint_target_pos.FL_hip_pos = clamped_actions[0][0].item<double>();
        // this->joint_target_pos.FL_hip_pos = clamped_actions[0][1].item<double>();
        // this->joint_target_pos.FL_hip_pos = clamped_actions[0][2].item<double>();
        // this->joint_target_pos.FR_hip_pos = clamped_actions[0][3].item<double>();
        // this->joint_target_pos.FR_hip_pos = clamped_actions[0][4].item<double>();
        // this->joint_target_pos.FR_hip_pos = clamped_actions[0][5].item<double>();
        //
        // this->joint_target_pos.RL_hip_pos = clamped_actions[0][6].item<double>();
        // this->joint_target_pos.RL_hip_pos = clamped_actions[0][7].item<double>();
        // this->joint_target_pos.RL_hip_pos = clamped_actions[0][8].item<double>();
        // this->joint_target_pos.RR_hip_pos = clamped_actions[0][9].item<double>();
        // this->joint_target_pos.RR_hip_pos = clamped_actions[0][10].item<double>();
        // this->joint_target_pos.RR_hip_pos = clamped_actions[0][11].item<double>();

        this->joint_target_pos_publisher.publish(this->joint_target_pos);
#endif
    }
}
